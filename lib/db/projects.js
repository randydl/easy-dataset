...
export async function deleteProject(projectId) {
  try {
    ...
    const delDatasets = db.datasets.deleteMany({ where: { projectId } });
    const delQuestions = db.questions.deleteMany({ where: { projectId } });
    const delChunk = db.chunks.deleteMany({ where: { projectId } });
    const delProject = db.projects.delete({ where: { id: projectId } });
    await prisma.$transaction([delDatasets, delQuestions, delChunk, delProject]);
