...
import { getUploadFilesByProjectId } from '@/lib/db/upload-files';
...
export async function splitProjectFile(projectId, fileName) {
  try {
    ...
    // 获取文件列表
    const files = await getUploadFilesByProjectId(projectId);
    ...
    for (let i = 0; i < files.length; i++) {
      // 检查文件是否存在
      let filePath = path.join(files[i].path, files[i].fileName);
      if (files[i].fileExt !== '.md') {
        filePath = path.join(files[i].path, files[i].fileName.replace(/\.[^/.]+$/, '.md'));
      }
      try {
        await fs.promises.access(filePath);
      } catch (error) {
        throw new Error(`文件 ${files[i].fileName} 不存在`);
      }
      ...
      let data = splitResult.map((part, index) => {
        const chunkId = `${path.basename(files[i].fileName, path.extname(files[i].fileName))}-part-${index + 1}`;
        return {
          projectId,
          name: chunkId,
          fileId: files[i].id,
