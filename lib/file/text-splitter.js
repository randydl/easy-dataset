...
async function splitFileByType({ splitType, projectPath, fileContent, fileName, projectId, fileId }) {
  // 获取任务配置
  ...
  const separators = taskConfig.separator || ['|', '##', '>', '-'];
 ...
 */
export async function splitProjectFile(projectId, file, splitType) {
  ...
  try {
    ...
    // 保存分割结果到chunks目录
    const savedChunks = await splitFileByType({ splitType, projectPath, fileContent, fileName, projectId, fileId });
